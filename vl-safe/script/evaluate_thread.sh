...
show_help() {
    ...
    echo "  --max-samples N            每个数据集最大样本数（默认：10000）"
    echo "  --concurrency N            并发线程数（默认：8）"
    echo "  --reasoning-effort LEVEL   推理强度：low/medium/high（默认：low）"
    echo "  --max-tokens N             最大token数（默认：128）"
