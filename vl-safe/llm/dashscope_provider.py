...
class DashScopeProvider(BaseLLMProvider):
    ...
    def __init__(self, api_key: Optional[str] = None, base_url: Optional[str] = None, **kwargs):
        ...
        Args:
            api_key: DashScope API密钥，如果不提供则从环境变量DASHSCOPE_API_KEY读取
            base_url: API基础URL，默认为DashScope的OpenAI兼容端点
            **kwargs: 其他配置参数
        """
        ...
        # 如果提供了api_key，使用提供的key，否则从环境变量DASHSCOPE_API_KEY读取
        if self.api_key:
            self.client = OpenAI(
    ...
    def _validate_video_input_mode(self, video_input_mode: Optional[str]) -> str:
        ...
        if video_input_mode not in ["base64", "frames"]:
            if video_input_mode == "upload":
                warnings.warn(
                    "警告: DashScope不支持'upload'模式，将使用默认的'base64'模式。",
                    UserWarning
                )
                return "base64"
            else:
                    ...
                    if isinstance(item, dict):
                        ...
                        # 处理video_url: 支持base64和frames模式
                            ...
                            if video_input_mode == "frames":
                                ...
                                # base64模式：视频直接转base64（DashScope限制10MB）
                                if is_local_file(url):
                                    # 检查并压缩视频
    ...
    def completion(
        ...
        **kwargs
        ...
        Args:
            ...
            thinking_budget: 思考过程的最大Token数，默认81920
